...
async def imagine_slash(interaction: discord.Interaction, prompt: str):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post("https://api-inference.huggingface.co/models/stabilityai/stable-diffusion-xl-base-1.0",
                                  headers=headers, json=payload, timeout=60) as resp:
                if resp.status!= 200:
                    await interaction.followup.send(f"HF Error {resp.status}: Model might be loading. Try again in 20s.")
                    return
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post("https://api-inference.huggingface.co/models/stabilityai/stable-diffusion-xl-base-1.0",
                                  headers=headers, json=payload, timeout=60) as resp:
                if resp.status!= 200:
                    await msg.edit(content=f"HF Error {resp.status}: Model might be loading. Try again in 20s.")
                    return
                img_bytes = await resp.read()
